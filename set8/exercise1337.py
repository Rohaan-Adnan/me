...
def set_it_on_fire(input_string="very naughty boy") -> str:
    ...
    """
    input_string = input_string.upper()  # Convert the input string to uppercase
    chars = list(input_string)  # Convert the string into a list of characters
    fire_emoji = "🔥"
    interleaved_string = fire_emoji.join(chars)  # Join the characters with the fire emoji
    return f"{fire_emoji}{interleaved_string}{fire_emoji}"  # Add fire emojis at both ends of the string
...
def best_letter_for_pets() -> str:
    ...
    for letter in the_alphabet:
        ...
        pet_count = len(pets_with_letter)
...
def make_filler_text_dictionary() -> Dict:
    ...
    for word_length in range(3, 8):
        words_list = []
        for _ in range(4):  # Get 4 words for each word_length
            r = requests.get(url + str(word_length))
            words_list.append(r.text.strip())  # Add the fetched word to the list
